...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, url) in urls.iter().enumerate() {
        ...
        match md.convert_url(url).await {
            Ok(markdown) => {
                ...
                println!("      ✅ Success: {} chars in {:?}", char_count, duration);
    ...
    println!("   📊 Sequential processing completed in {:?}\n", sequential_duration);
            ...
            Ok(result) => {
                match result {
                    Ok((url, markdown, duration)) => {
                        println!("   [{}/{}] ✅ {}: {} chars in {:?}", 
                            i + 1, urls.len(), url, markdown.as_str().len(), duration);
                        parallel_results.push(Ok((url, markdown, duration)));
                    }
                    Err((url, e, duration)) => {
                        println!("   [{}/{}] ❌ {}: Failed in {:?} - {}", 
                            i + 1, urls.len(), url, duration, e);
                        parallel_results.push(Err((url, e, duration)));
                    }
                }
            }
            Err(join_error) => {
                println!("   [{}/{}] 💥 Task failed: {}", i + 1, urls.len(), join_error);
            }
        }
    }
    
    let parallel_duration = parallel_start.elapsed();
    println!("   📊 Parallel processing completed in {:?}\n", parallel_duration);
    ...
    for (i, url) in urls.iter().enumerate() {
        println!("   [{}/{}] Processing with timeout: {}", i + 1, urls.len(), url);
            ...
            loop {
                ...
                match md.convert_url(url).await {
                    Ok(markdown) => {
                        ...
                        println!("      ✅ Success on attempt {} in {:?}", attempts, duration);
                    ...
                    Err(e) => {
                        ...
                        if attempts < max_attempts && e.is_retryable() {
                            println!("      🔄 Attempt {} failed in {:?}, retrying: {}", attempts, duration, e);
                            tokio::time::sleep(Duration::from_millis(1000 * attempts as u64)).await;
                            continue;
                        } else {
                            println!("      ❌ Failed after {} attempts in {:?}: {}", attempts, duration, e);
        ...
        match result {
            ...
            Err(_timeout_error) => {
                ...
                advanced_results.push(Err((url.to_string(), timeout_error, Duration::from_secs(10), 1)));
            }
        }
    }
    
    let advanced_duration = advanced_start.elapsed();
    println!("   📊 Advanced processing completed in {:?}\n", advanced_duration);
    ...
    let seq_total_chars: usize = sequential_results.iter()
    ...
    println!("      Success Rate: {}/{} ({:.1}%)", 
        seq_success_count, urls.len(), 
        (seq_success_count as f32 / urls.len() as f32) * 100.0);
    println!("      Total Content: {} characters", seq_total_chars);
    ...
    let par_total_chars: usize = parallel_results.iter()
        .filter_map(|r| r.as_ref().ok())
        .map(|(_, markdown, _)| markdown.as_str().len())
        .sum();
    ...
    println!("      Success Rate: {}/{} ({:.1}%)", 
        par_success_count, urls.len(), 
        (par_success_count as f32 / urls.len() as f32) * 100.0);
    println!("      Total Content: {} characters", par_total_chars);
    ...
    println!("   • Parallel processing: {:.1}x faster for I/O bound operations", 
        sequential_duration.as_secs_f32() / parallel_duration.as_secs_f32());
