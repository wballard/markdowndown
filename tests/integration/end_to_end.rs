...
async fn test_end_to_end_workflow() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let test_cases = [
        ...
        ("https://en.wikipedia.org/wiki/Rust_(programming_language)", "HTML", "Complex Wikipedia page"),
        ("https://github.com/rust-lang/rust/issues/1", "GitHub", "GitHub issue conversion"),
        ("https://docs.google.com/document/d/1BxiMVs0XRA5nFMdKvBdBZjgmUUqptlbs74OgvE2upms/edit", "Google Docs", "Google Docs conversion"),
    ];
    ...
    for (url, service_type, description) in test_cases.iter() {
        ...
        match result {
            Ok(markdown) => {
                ...
                match *service_type {
                    "HTML" => {
                        assert!(content.contains('#') || content.len() > 200,
                ...
                println!("  ✓ Success: {} chars in {:?}", content_length, conversion_duration);
            }
            Err(e) => {
                println!("  ⚠ Failed: {} (may be acceptable for some services)", e);
    ...
    println!("  Total content generated: {} chars", total_chars);
    println!("  Total time: {:?}", total_duration);
    println!("  Average time per test: {:?}", total_duration / test_cases.len() as u32);
    
    // Should have at least some successful conversions
    assert!(successful_conversions > 0, "At least one conversion should succeed in end-to-end test");
...
async fn test_cross_service_fallback() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let fallback_test_cases = [
        ("https://docs.google.com/document/d/nonexistent/edit", "Non-existent Google Doc should fallback to HTML"),
        ("https://github.com/nonexistent/repo/issues/1", "Non-existent GitHub issue might fallback"),
    ];
    ...
    for (url, description) in fallback_test_cases.iter() {
        ...
        match result {
            ...
            Err(e) => {
                println!("  ⚠ Fallback failed: {} (acceptable)", e);
...
async fn test_configuration_propagation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(result.frontmatter().is_some(), "Custom config should include frontmatter");
    
    let frontmatter = result.frontmatter().unwrap();
    assert!(frontmatter.contains("test_run: integration"),
           "Should include custom frontmatter field: test_run");
    assert!(frontmatter.contains("config_test: \"true\"") || frontmatter.contains("config_test: true"),
...
async fn test_concurrent_cross_service_processing() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let futures = concurrent_urls.iter().enumerate().map(|(i, url)| async move {
    ...
    for (url, result) in results {
        match result {
            Ok(markdown) => {
                ...
                println!("  ✓ Concurrent success for {}: {} chars", url, content_length);
            ...
            Err(e) => {
                println!("  ⚠ Concurrent failure for {}: {} (may be acceptable)", url, e);
            }
        }
    }
    ...
    assert!(successful > 0, "At least one concurrent conversion should succeed");
...
async fn test_library_metadata_consistency() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !config.skip_external_services {
        ...
        assert!(frontmatter.contains("converted_at"), "Should include conversion timestamp");
        assert!(frontmatter.contains("source_url"), "Should include source URL");
        assert!(frontmatter.contains("conversion_type") || frontmatter.contains("html2markdown"),
               "Should include conversion type");
        
        // User agent should be consistent
        let user_agent_field = format!("user_agent:");
        if frontmatter.contains(&user_agent_field) {
            assert!(frontmatter.contains("markdowndown/"),
...
async fn test_error_propagation_and_recovery() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mixed_urls = [
        ...
        ("https://httpbin.org/status/404", false, "Should fail with 404"),
        ("https://invalid-domain-12345.com", false, "Should fail with DNS error"),
        ("https://en.wikipedia.org/wiki/Rust_(programming_language)", true, "Should succeed"),
    ];
    ...
    for (url, should_succeed, description) in mixed_urls.iter() {
        ...
        match (result, should_succeed) {
            ...
            (Err(e), false) => {
                ...
                assert!(!e.to_string().is_empty(), "Error should have descriptive message");
                assert!(e.to_string().len() > 10, "Error message should be substantial");
            }
            (Err(e), true) => {
    ...
    assert!(successes + expected_failures >= mixed_urls.len() / 2,
...
async fn test_resource_management() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if config.skip_external_services || config.skip_slow_tests {
        println!("Skipping resource management test - external services disabled or slow tests skipped");
        return Ok(());
    }
    ...
    println!("Initial MarkdownDown instance size: {} bytes", initial_memory);
    ...
    for i in 0..num_conversions {
        println!("Resource test conversion {}/{}", i + 1, num_conversions);
        ...
        match result {
            Ok(markdown) => {
                ...
                println!("  Conversion {}: {} chars", i + 1, content_length);
                
                // Force drop of the markdown result to test cleanup
                drop(markdown);
            }
            Err(e) => {
                println!("  Conversion {} failed: {} (acceptable)", i + 1, e);
            }
        }
    }
    ...
    println!("  Total content processed: {} chars", total_content_length);
    println!("  Instance memory footprint: {} bytes", initial_memory);
    
    // The MarkdownDown instance should be reusable
    assert!(total_content_length > 0, "Should have processed some content");
