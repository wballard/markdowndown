    ...
    for (url, description) in TestUrls::GITHUB_TEST_URLS.iter() {
        ...
        match result {
            Ok(markdown) => {
                ...
                assert!(frontmatter.contains("github.com"), "Should reference GitHub in frontmatter");
...
async fn test_github_issue_conversion() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match result {
        Ok(markdown) => {
            ...
            println!("✓ GitHub issue #1 converted successfully ({} chars)", content.len());
        }
        Err(e) => {
...
async fn test_github_pull_request_conversion() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match result {
        Ok(markdown) => {
            ...
            println!("✓ GitHub PR #2000 converted successfully ({} chars)", content.len());
        }
        Err(e) => {
...
async fn test_github_url_detection() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for url in github_urls.iter() {
        ...
        assert_eq!(detected_type, markdowndown::types::UrlType::GitHubIssue,
...
async fn test_github_rate_limiting() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(successes + rate_limited == urls.len(), 
...
async fn test_github_error_scenarios() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let error_cases = [
        ...
        ("https://github.com/rust-lang/rust/issues/999999", "Non-existent issue"),
        ("https://github.com/rust-lang/rust/pull/999999", "Non-existent pull request"),
    ];
...
async fn test_github_performance() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for url in test_urls.iter() {
        ...
        match result {
            ...
            Err(e) => {
                println!("  Failed: {} (may be acceptable)", e);
