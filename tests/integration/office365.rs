...
async fn test_office365_conversions() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !config.can_test_office365() {
        println!(
            "Skipping Office 365 tests - no credentials available or external services disabled"
        );
        return Ok(());
    }
    ...
    // Note: These tests use example SharePoint URLs. For real testing, replace with actual accessible documents.
    // To test with real documents, set environment variables:
    // - OFFICE365_TEST_DOCX_URL: URL to a test Word document
    // - OFFICE365_TEST_XLSX_URL: URL to a test Excel document
    // - OFFICE365_TEST_PPTX_URL: URL to a test PowerPoint document
    let test_documents = [
        (
            std::env::var("OFFICE365_TEST_DOCX_URL").unwrap_or_else(|_| {
                "https://example.sharepoint.com/sites/public/Documents/test.docx".to_string()
            }),
            "Word document",
        ),
        (
            std::env::var("OFFICE365_TEST_XLSX_URL").unwrap_or_else(|_| {
                "https://example.sharepoint.com/sites/public/Documents/test.xlsx".to_string()
            }),
            "Excel spreadsheet",
        ),
        (
            std::env::var("OFFICE365_TEST_PPTX_URL").unwrap_or_else(|_| {
                "https://example.sharepoint.com/sites/public/Documents/test.pptx".to_string()
            }),
    ...
    for (url, description) in test_documents.iter() {
        ...
        match result {
            Ok(markdown) => {
                ...
                assert!(
                    frontmatter.contains("sharepoint") || frontmatter.contains("office365"),
                    "Should reference Office 365/SharePoint in frontmatter"
                );

                // Performance check
                assert!(
                    duration < config.large_document_timeout(),
            ...
            Err(e) => {
                println!("⚠ {description} failed (may be expected for placeholder URLs): {e}");
...
async fn test_office365_url_detection() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let office365_urls = [
        "https://example.sharepoint.com/sites/team/Documents/Document.docx",
        "https://example-my.sharepoint.com/personal/user/Documents/Workbook.xlsx",
        "https://example.sharepoint.com/sites/project/Lists/Tasks/AllItems.aspx",
    ...
    for url in office365_urls.iter() {
        ...
        assert_eq!(
            detected_type,
            markdowndown::types::UrlType::Office365,
...
async fn test_office365_authentication() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let test_url = "https://example.sharepoint.com/sites/public/Shared%20Documents/test.docx";
...
async fn test_office365_document_types() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let document_types = [
        (
            "https://example.sharepoint.com/sites/public/Shared%20Documents/test.docx",
            "Word Document",
            "docx",
        ),
        (
            "https://example.sharepoint.com/sites/public/Shared%20Documents/test.xlsx",
            "Excel Spreadsheet",
            "xlsx",
        ),
        (
            "https://example.sharepoint.com/sites/public/Shared%20Documents/test.pptx",
            "PowerPoint Presentation",
            "pptx",
        ),
        (
            "https://example.sharepoint.com/sites/public/Shared%20Documents/test.pdf",
            "PDF Document",
            "pdf",
        ),
    ];
    ...
    for (url, doc_type, extension) in document_types.iter() {
        println!("Testing {doc_type} type: {url}");
        ...
        match result {
            Ok(markdown) => {
                let content = markdown.as_str();
                println!(
                    "  ✓ {doc_type} converted successfully ({} chars)",
                    content.len()
                );
                ...
                assert!(
                    TestUtils::validate_markdown_quality(content),
                    "Poor quality conversion for {doc_type} type"
                );
                ...
                assert!(
                    frontmatter.contains(extension),
                    "Frontmatter should indicate document type: {extension}"
                );
            }
            Err(e) => {
                println!("  ⚠ {doc_type} conversion failed (expected for placeholder URLs): {e}");
...
async fn test_office365_error_scenarios() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let error_cases = [
        (
            "https://example.sharepoint.com/sites/nonexistent/document.docx",
            "Non-existent site",
        ),
        (
            "https://example.sharepoint.com/sites/public/nonexistent.docx",
            "Non-existent document",
        ),
        (
            "https://invalid.sharepoint.com/sites/test/document.docx",
            "Invalid domain",
        ),
    ];
...
async fn test_sharepoint_lists_and_libraries() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let sharepoint_urls = [
        ...
        (
            "https://example.sharepoint.com/sites/team/Lists/Tasks/AllItems.aspx",
            "Tasks List",
        ),
        (
            "https://example.sharepoint.com/sites/team/Lists/Announcements",
            "Announcements List",
        ),
    ];
    ...
    for (url, description) in sharepoint_urls.iter() {
        ...
        match result {
            Ok(markdown) => {
                let content = markdown.as_str();
                println!(
                    "  ✓ {description} converted successfully ({} chars)",
                    content.len()
                );
            ...
            Err(e) => {
                println!(
                    "  ⚠ {description} conversion failed (expected for placeholder URLs): {e}"
                );
                assert!(!e.to_string().is_empty());
            }
        }
...
async fn test_office365_performance() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let test_documents = [
        (
            "https://example.sharepoint.com/sites/public/Shared%20Documents/small.docx",
            "Small Document",
        ),
        (
            "https://example.sharepoint.com/sites/public/Shared%20Documents/large.docx",
            "Large Document",
        ),
    ];
    ...
    for (url, description) in test_documents.iter() {
        ...
        match result {
            ...
            Err(e) => {
                println!("  Failed: {e} (expected for placeholder URLs)");
                // For placeholder URLs, failures are expected
                assert!(!e.to_string().is_empty());
            }
        }
...
async fn test_onedrive_urls() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !config.can_test_office365() {
        println!(
            "Skipping OneDrive tests - no credentials available or external services disabled"
        );
        return Ok(());
    }
    ...
    let onedrive_urls = [
        ...
        "https://example-my.sharepoint.com/personal/user_company_com/Documents/test.docx",
