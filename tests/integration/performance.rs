...
async fn test_rate_limiting_behavior() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, url) in test_urls.iter().enumerate() {
        ...
        println!("Rate limiting test request {}: {}", i + 1, url);
        ...
        match result {
            Ok(markdown) => {
                successes += 1;
                println!("  ✓ Success: {} chars in {:?}", markdown.as_str().len(), request_duration);
                assert!(TestUtils::validate_markdown_quality(markdown.as_str()));
            }
            Err(e) => {
                println!("  ⚠ Failed: {} in {:?}", e, request_duration);
    ...
    println!("  Average time per request: {:?}", total_duration / test_urls.len() as u32);
    
    // Validate rate limiting behavior
    for (i, duration) in request_times.iter().enumerate() {
        println!("  Request {} duration: {:?}", i + 1, duration);
    }
    
    // Should demonstrate controlled request timing
    if config.request_delay_ms > 0 && request_times.len() > 1 {
        let total_expected_delay = Duration::from_millis(config.request_delay_ms * (request_times.len() as u64 - 1));
        assert!(total_duration >= total_expected_delay,
...
async fn test_comprehensive_performance_benchmark() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let benchmark_cases = [
        ("https://httpbin.org/html", "HTML", "Small", "Simple HTML page"),
        ("https://en.wikipedia.org/wiki/Rust_(programming_language)", "HTML", "Large", "Complex Wikipedia page"),
        ("https://doc.rust-lang.org/book/ch01-00-getting-started.html", "HTML", "Medium", "Documentation page"),
    ];
    ...
    for (url, service_type, size_category, description) in benchmark_cases.iter() {
        ...
        match result {
            Ok(markdown) => {
                ...
                println!("  ✓ Speed: {:.2} chars/sec", chars_per_second);
                ...
                assert!(duration < max_duration,
                       "Conversion took too long for {} content: {:?} > {:?}", 
                       size_category, duration, max_duration);
                
                // Quality validation
                assert!(TestUtils::validate_markdown_quality(content),
                       "Performance test should maintain quality");
                
                results.push((*service_type, *size_category, duration, content_length, chars_per_second));
            }
            Err(e) => {
                println!("  ⚠ Failed: {} (may affect benchmark)", e);
    ...
    for (service, size, duration, content_length, speed) in results.iter() {
        println!("│ {:11} │ {:8} │ {:11?} │ {:11} │ {:15.2} │", 
                service, size, duration, content_length, speed);
    }
    ...
    if !successful_results.is_empty() {
        let total_duration: Duration = successful_results.iter()
            .map(|(_, _, duration, _, _)| *duration)
            .sum();
        let total_content: usize = successful_results.iter()
            .map(|(_, _, _, content_length, _)| *content_length)
            .sum();
        let avg_speed: f64 = successful_results.iter()
            .map(|(_, _, _, _, speed)| *speed)
            .sum::<f64>() / successful_results.len() as f64;
        
        println!("\nAggregate Performance Statistics:");
        println!("  Successful conversions: {}/{}", successful_results.len(), results.len());
        println!("  Total processing time: {:?}", total_duration);
        println!("  Total content processed: {} chars", total_content);
        println!("  Average conversion speed: {:.2} chars/sec", avg_speed);
        println!("  Overall throughput: {:.2} chars/sec", 
...
async fn test_timeout_and_large_document_handling() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let timeout_test_urls = [
        ...
        ("https://en.wikipedia.org/wiki/Rust_(programming_language)", "Potentially slow response"),
    ];
    ...
    for (url, description) in timeout_test_urls.iter() {
        ...
        match short_result {
            Ok(markdown) => {
                println!("    ✓ Completed within timeout: {:?} ({} chars)", 
                        short_duration, markdown.as_str().len());
                assert!(short_duration < Duration::from_secs(6),
                       "Should complete within timeout period");
                assert!(TestUtils::validate_markdown_quality(markdown.as_str()));
            }
            Err(e) => {
                println!("    ⚠ Timeout or error: {} in {:?}", e, short_duration);
                if e.to_string().contains("timeout") {
                    println!("      Timeout behavior working correctly");
                    // Timeout should occur reasonably close to the configured limit  
                    assert!(short_duration >= Duration::from_secs(4) && short_duration <= Duration::from_secs(10),
        ...
        match long_result {
            Ok(markdown) => {
                println!("    ✓ Completed: {:?} ({} chars)", 
                        long_duration, markdown.as_str().len());
                assert!(TestUtils::validate_markdown_quality(markdown.as_str()));
                
                // Long timeout should not actually take 120 seconds for simple URLs
                assert!(long_duration < Duration::from_secs(60),
                       "Should complete well before timeout for simple URLs");
            }
            Err(e) => {
                println!("    ⚠ Failed even with long timeout: {} in {:?}", e, long_duration);
                // Should not be a timeout error with long timeout
                assert!(!e.to_string().contains("timeout") || long_duration >= Duration::from_secs(100),
...
async fn test_error_scenario_performance() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (url, description) in error_scenarios.iter() {
        ...
        match result {
            Ok(markdown) => {
                println!("  ✓ Unexpected success: {} chars in {:?}", 
                        markdown.as_str().len(), duration);
    ...
    for (description, duration, success) in error_timings.iter() {
        ...
        println!("│ {:27} │ {:11?} │ {:7} │", description, duration, result_str);
    }
    
    println!("└─────────────────────────────┴─────────────┴─────────┘");
    
    // Calculate average error detection time
    let error_durations: Vec<Duration> = error_timings.iter()
        .filter(|(_, _, success)| !success)
        .map(|(_, duration, _)| *duration)
        .collect();
    
    if !error_durations.is_empty() {
        let avg_error_time: Duration = error_durations.iter().sum::<Duration>() / error_durations.len() as u32;
        println!("Average error detection time: {:?}", avg_error_time);
        
        // Most errors should be detected within reasonable time
        let quick_errors = error_durations.iter()
            .filter(|d| **d < Duration::from_secs(30))
            .count();
        
        println!("Quick error detection (< 30s): {}/{}", quick_errors, error_durations.len());
    }
...
async fn test_memory_and_resource_efficiency() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if config.skip_external_services || config.skip_slow_tests {
        println!("Skipping memory efficiency test - external services disabled or slow tests skipped");
        return Ok(());
    }
    ...
    println!("Testing resource efficiency over {} iterations", num_iterations);
    ...
    for i in 0..num_iterations {
        ...
        match result {
            Ok(markdown) => {
                ...
                println!("  Iteration {}: {} chars in {:?}", i + 1, content_size, iteration_duration);
    ...
    if !content_sizes.is_empty() && !processing_times.is_empty() {
        ...
        println!("  Successful iterations: {}/{}", content_sizes.len(), num_iterations);
        ...
        println!("  Time variance ratio: {:.2}x", time_variance);
        
        // Processing times should be reasonably consistent (within 3x variance)
        assert!(time_variance < 3.0,
